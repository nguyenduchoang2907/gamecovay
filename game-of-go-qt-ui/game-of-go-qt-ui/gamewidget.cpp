...
#include <QDebug>
#include <QTimer>
// Hàm khởi tạo GameWidget với các thông số cần thiết để tạo trò chơi.
...
GameWidget::GameWidget(
    ...
    , myColor(myColor)
    , myTurn(myColor == 1)// Xác định lượt chơi dựa trên màu quân của người chơi
    , gameFinished(false)// Biến để theo dõi trạng thái kết thúc trò chơi
    , lastCoords("")// Lưu vị trí cuối cùng được chơi trên bàn cờ
    , lastColor(0)// Màu của nước cờ cuối cùng
    , timeSystem(timeSystem)//
    , byoyomiTime(byoyomiTime)//?
    , timer(new QTimer(this))// Đồng hồ để đếm thời gian
    , socket(Socket::getInstance())
{
    ui->setupUi(this);// Khởi tạo giao diện người dùng
        // Tạo và cấu hình các thành phần của trò chơi như bàn cờ, bảng điểm và bảng log
    ...
    // Thiết lập trạng thái ban đầu cho nút "Pass" và label hiển thị thông tin lượt chơi?
...
void GameWidget::onGameBoardClicked(QString coords) {
    socket->sendMessage("MOVE", QString("%1\n%2\n").arg(myColor).arg(coords));    // Gửi tin nhắn tới socket với thông tin nước cờ mới được đánh
   
    // Nếu có hệ thống thời gian, gửi tin nhắn về việc quản lý thời gian (BYOYOM)
    if (timeSystem == 1) {
        usleep(1000);// Tạm ngừng một giây
        socket->sendMessage("BYOYOM", QString("%1\n%2\n%3\n").arg(myColor).arg(timingType).arg(timeLeft));
    }
}
// Xử lý khi nhận được message từ socket
...
void GameWidget::on_btn_pass_clicked()
{
    if (QMessageBox::question(
            ...
            "Are you sure you want to pass?"
                ...
                // Xóa quân cờ vẽ lần chơi trước (nếu có)
        ...
        lastCoords = "";
        lastColor = 0;
        // Gửi tin nhắn MOVE đến socket thông báo người chơi Pass
